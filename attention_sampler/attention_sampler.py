...
class AttSampler:
    ...
    def forward(self, data, attx, atty):
        ...
        threshold = self.scale * self.dense * in_size * att_size
        attx = attx * out_size
        atty = atty * out_size
        for j in range(self.iters):
            ...
            if j == 0:
                threshold = self.F.maximum(self.F.maximum(max_attx, max_atty), threshold) # (N, 1)
            else:
        ...
        mobula.func.map_step(N, attxi, index_x, stepx, att_size, out_size)
        mobula.func.map_step(N, attyi, index_y, stepy, att_size, out_size)
        grid = self.F.stack(
